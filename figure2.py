...
df = pd.read_excel('fire time range from 2022-10 to 2023-09.xlsx')  # Replace with the path to the dataset file

# Group by 'fire_zone' and 'type_description' and count the occurrences
grouped = df.groupby(['fire_zone', 'type_description']).size().unstack(fill_value=0)

# Get the top 10 fire zones based on the total count of incidents
top_10_fire_zones = grouped.sum(axis=1).nlargest(10).index

# Filter the dataset to include only the top 10 fire zones
top_10_data = grouped.loc[top_10_fire_zones]

# Sort the fire zones by the total incidents within them in descending order for the plot
